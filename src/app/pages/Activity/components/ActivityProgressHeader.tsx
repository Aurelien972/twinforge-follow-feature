...
// Types pour la pipeline d'activité
...
const ActivityProgressHeaderInner: React.FC<ActivityProgressHeaderProps> = ({
  isActive,
  currentStep,
  overallProgress,
  message,
  ...
  // Couleurs spécifiques à la Forge Énergétique (training)
  const forgeColors = {
    primary: '#3B82F6',      // Bleu saillant
    secondary: '#06B6D4',    // Bleu cyan
    accent: '#1D4ED8',       // Bleu profond
    glow: 'rgba(59, 130, 246, 0.4)',
    glowEnhanced: 'rgba(59, 130, 246, 0.6)',
  };
  ...
  React.useEffect(() => {
    if (overallProgress != null && (!Number.isFinite(overallProgress) || Number.isNaN(overallProgress))) {
      logger.error('ACTIVITY_PROGRESS_HEADER', 'Invalid progress detected', { 
  ...
  React.useEffect(() => {
    ...
    if (!last || last.step !== current.step || last.progress !== current.progress) {
      ...
      logger.info('ACTIVITY_PROGRESS_HEADER', 'Forge progress update', {
        currentStep,
  ...
  }, [currentStep, safeProgress, overallProgress, message, subMessage, steps.length, currentStepIndex]);
  ...
  return (
    <div className="w-full orbe-entrance-animation mt-2 md:mt-2">
      <GlassCard
        className="activity-progress-container relative overflow-visible p-3 md:p-4 lg:p-5 w-full"
        style={{
          background: `
            ...
            rgba(11, 14, 23, 0.75)
          `,
          borderColor: `color-mix(in srgb, ${forgeColors.secondary} 35%, transparent)`,
          boxShadow: `
      ...
      >
        ...
        <div className="relative z-10 flex items-center gap-3 md:gap-3">
          {/* Colonne gauche : Icône de la Forge Énergétique */}
          <div className="relative flex-shrink-0" style={{ width: 'fit-content' }}>
            <div
              ...
              style={{
                ...
                background: `
                  ...
                  radial-gradient(circle at 70% 70%, color-mix(in srgb, ${forgeColors.secondary} 20%, transparent) 0%, transparent 50%),
                ...
                `,
                border: `3px solid color-mix(in srgb, ${forgeColors.secondary} 70%, transparent)`,
                boxShadow: `
                  0 0 ${isMobile ? '24px' : '36px'} color-mix(in srgb, ${forgeColors.secondary} 60%, transparent),
                  0 0 ${isMobile ? '40px' : '64px'} color-mix(in srgb, ${forgeColors.secondary} 40%, transparent),
            ...
            >
              <SpatialIcon
                Icon={ICONS[currentStepData?.icon] || ICONS.Activity}
                size={iconSize}
                style={{
                  color: forgeColors.secondary,
                  filter: `
                    drop-shadow(0 0 8px color-mix(in srgb, ${forgeColors.secondary} 80%, transparent))
                    drop-shadow(0 0 16px color-mix(in srgb, ${forgeColors.secondary} 60%, transparent))
                  `
                }}
                glowColor={forgeColors.secondary}
                variant="pure"
                aria-hidden
              />
            </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="space-y-[6px] md:space-y-2">
              <h2
                ...
                style={{
                  ...
                  textShadow: `0 0 6px color-mix(in srgb, ${forgeColors.secondary} 14%, transparent)`
                }}
              >
                {message}
              </h2>
            ...
            <div className="mt-1.5 flex items-center gap-2 flex-wrap">
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        border: isCurrent
                          ? `2px solid color-mix(in srgb, ${forgeColors.secondary} 80%, transparent)`
                          : isCompleted
                          ? `2px solid color-mix(in srgb, ${forgeColors.primary} 70%, transparent)`
                          : `1px solid color-mix(in srgb, ${forgeColors.primary} 50%, transparent)`,
                        position: 'relative',
                        transition: 'all 220ms cubic-bezier(0.2, 0.8, 0.2, 1)',
                        boxShadow: isCurrent
                          ? `0 0 10px color-mix(in srgb, ${forgeColors.secondary} 55%, transparent)`
                          : isCompleted
                          ? `0 0 9px color-mix(in srgb, ${forgeColors.primary} 45%, transparent)`
                          : 'none'
                      }}
                    />
                  );
              ...
              {/* % — MOBILE SEULEMENT */}
              <div className="text-right font-bold md:hidden" style={{ color: forgeColors.secondary, fontSize: 14 }}>
          ...
          {/* Anneau de progression énergétique — Desktop */}
          <div className="relative flex-shrink-0 hidden md:flex" style={{ width: '52px', height: '52px' }} aria-hidden>
            <div
              className="absolute inset-0 rounded-full"
              style={{
                background: `conic-gradient(from -90deg,
                  ${forgeColors.secondary} 0%,
                  ${forgeColors.secondary} ${safeProgress * 3.6}deg,
                  color-mix(in srgb, ${forgeColors.primary} 40%, transparent) ${safeProgress * 3.6}deg,
                  color-mix(in srgb, ${forgeColors.primary} 40%, transparent) 360deg
                )`,
                ...
                boxShadow: `0 0 18px color-mix(in srgb, ${forgeColors.secondary} 48%, transparent), 0 0 36px color-mix(in srgb, ${forgeColors.secondary} 28%, transparent)`,
              }}
            />
            <div className="absolute inset-0 flex items-center justify-center" style={{ color: forgeColors.secondary, fontSize: 15, fontWeight: 800 }}>
              {Math.round(safeProgress)}%
            </div>
          </div>
        </div>
...
const ActivityProgressHeader = React.memo(ActivityProgressHeaderInner);
export default ActivityProgressHeader;